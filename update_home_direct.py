...
for i in range(len(lines)):
    if '{/* PRODUCTS SECTION */}' in lines[i]:
        idx_products = i
        break

# The lines before `idx_products` should be:
# lines[idx_products-1] is `        )}`
# lines[idx_products-2] is `        </div>` (the closing of the hero div)
...
# Let's replace the `        </div>` and `        )}` with the new block.
lines = lines[:idx_products-2] + new_featured_html.split('\n') + lines[idx_products:]

# We also need to fix the `div` wrapper for homeRef
for i in range(len(lines)):
    if '{activeSection === "home" && (' in lines[i]:
        # The next line should be the `<div ref={homeRef} className="relative bg-teal-900 text-white py-24 sm:py-32 overflow-hidden">`
        # Or `<div ref={homeRef}>` if it was already replaced.
        if 'className=' in lines[i+1]:
            lines[i+1] = '        <div ref={homeRef}>\n          <div className="relative bg-teal-900 text-white py-24 sm:py-32 overflow-hidden">'
        break
